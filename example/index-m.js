import VectorTileImageryProvider from '../src/index'
...
Cesium.Resource.fetchText("./Assets/Data/json/bj.json").then(function (geojson) {
    geojson = eval("(" + geojson + ")");
    var mask = null;

    try {
        //缓冲区
        var bufferedOuter = turf.buffer(geojson.features[0], 2, {
            units: "kilometers"
        });

        var bufferedInner = turf.buffer(geojson.features[0], 1, {
            units: "kilometers"
        })
        ...
        bufferedOuterProvider.readyPromise.then(function () {
            viewer.imageryLayers.addImageryProvider(bufferedOuterProvider);
        });
        ...
        bufferedInnerProvider.readyPromise.then(function () {
            viewer.imageryLayers.addImageryProvider(bufferedInnerProvider);
        });
    ...
    Cesium.Resource.fetchText("./Assets/Data/json/bjsx.json").then(function (bjsx) {
        bjsx = eval("(" + bjsx + ")");
